<script type="text/javascript">
    jQuery(document).ready(function($) {
        jQuery("#swfu_images").first().after('<a href="#" onclick="ls.photoset.showFormLink(); return false;" id="photoset-start-upload-link" class="button margin-5">{$aLang.plugin.remotephoto.add_link_text}</a><br />');
        ...
        $('#photoset-upload-form .modal-content').first().prepend('<div class="tabs-default"><ul class="horizontal"><li class="active js-block-upload-photo-item" data-type="pc"><a href="#">{$aLang.uploadimg_from_pc}</a></li><li class="js-block-upload-photo-item" data-type="link"><a href="#">{$aLang.uploadimg_from_link}</a></li></ul></div><div id="block_upload_photo_content_link" class="tab-content js-block-upload-photo-content" data-type="link" style="display: none;"><label for="photoset-upload-link">{$aLang.uploadimg_url}:</label><input type="text" name="photoset-upload-link" id="photoset-upload-link" value="http://" class="input-text input-width-full" /></div>');
